...
public class AdminService {
    ...
    public PageDto adminMview(String detail, int page, int tablerows, int[] state, String key, String keyword){
        ...
        else if (detail.equals("review")) {totalBoardSize = adminDao.getRVTableSize(key,keyword);}
        else if (detail.equals("store")) {totalBoardSize = adminDao.getSTableSize(state,key,keyword);}

        // 2. 총 페이지수 계산 (나머지 값 없으면 그대로, 나머지 있으면 +1)
        System.out.println("ddddd"+totalBoardSize);
        int totalPage = totalBoardSize % tablerows == 0 ?
                totalBoardSize/tablerows : totalBoardSize/tablerows+1;
        System.out.println("eddddd"+totalPage);
        // 4. 게시물 정보 요청
